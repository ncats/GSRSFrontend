...
import { Editor, Ketcher } from '@gsrs-core/structure-editor/structure.editor.model';
  ...
  */
  ...
  tabSelectedUpdated(event: MatTabChangeEvent) {
    if (event) {
      ...
      // LATER: this.storeCriteriaInLocalStorage();
      ...
      if (this.category) {
        ...
        // LATER: const tabSelectedHash = this.utilitiesService.hashCode(this.tabSelectedIndex);
        // LATER: this.getStoredCriteriaFromLocalStorage(tabSelectedHash);
  ...
  processSearch(): void {
    ...
    if ((this.query) || (Object.keys(this.privateFacetParams).length > 0)) {
      ...
      } else if ((this.privateFacetParams && Object.keys(this.privateFacetParams).length > 0)) {
