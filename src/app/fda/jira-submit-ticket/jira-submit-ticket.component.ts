...
export class JiraSubmitTicketComponent implements OnInit, AfterViewInit {
  ...
  initCollectorDialog() {
    const text =
      'window.ATL_JQ_PAGE_PROPS = {"triggerFunction": function (showCollectorDialog) {window.showCollectorDialog = showCollectorDialog}};';
    const node = document.createElement("script");
    node.type = "text/javascript";
    node.innerHTML = text;
    node.async = true;
