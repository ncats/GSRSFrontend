...
export class DataManagementComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.fileType = 'JSON';
    this.jobId = this.route.snapshot.queryParamMap.get('job') || null;
    if (this.jobId) {
  ...
  clearJob(): void {
    ...
    this.timeleft = null;
    this.max = 0;
    this.averagePersistRate = null;
    this.humanTimeTotal = null;
    this.humanTimeLeft = null;
    this.humanTimeEstimate = null;
    this.startedHuman = null;
    this.finishedHuman = null;
    this.jobId = null;
    this.monitor = true;
  }
  ...
  onSubmit() {
   ...
   this.stats.processedPass = 0;
    this.adminService.loadData(formData).pipe(take(1)).subscribe(response => {
      ...
      if (response.statistics && response.statistics.totalRecords) {
...
mixResultDisplay(job) {
  ...
  this.stats.processedPass = job.statistics.recordsProcessedFailed;
  this.test.t3 = this.test.t1 + this.test.f1;
}



toFullHumanTime(sent) {
  let result = '0';
  if (sent.years() > 0) {
