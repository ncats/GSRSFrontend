...
export class AdminService extends BaseHttpService {
  ...
  public fetchJobs(): Observable< any > {
    ...
    return this.http.get< any >(debugUrl); //`${url}scheduledjobs`
  }
  
  public fetchAdditionalJobs(): Observable<any> {
    const url = this.configService.configData.additionalJobUrls[0];
    console.log(`fetchAdditionalJobs using url ${url}`);
    ...
    public changePassword( newpass: string, id: number): Observable< any > {
      ...
      public changeMyPassword(oldpass: string, newpass: string, id: number): Observable< any > {
       ...
       /* public putAdapter(file: any): Observable< Auth > {
        ...
        public SearchStagedData(skip: any, facets?: any, term?: any, top?: any, view?: string) {
