...
export class SubstanceFormComponent implements OnInit, AfterViewInit, OnDestroy {
    @Input()
    simplifiedFormNameToggle = true // TODO:Set depending on config
  ...
  scrub(oldraw: any, importType?: string): any {
    ...
    toggleSimpleFormChanged(event: MatButtonToggleChange) {
        this.simplifiedFormNameToggle = event.source.checked
        this.updateHiddenFormSections()
    }

    updateHiddenFormSections(){
        let s = this.formSections.find(x => x.dynamicComponentName === 'substance-form-definition')
        if (s) {
            s.isHidden = !this.simplifiedFormNameToggle
        }
        s = this.formSections.find(x => x.dynamicComponentName === 'substance-form-names')
        if (s) {
            s.isHidden = !this.simplifiedFormNameToggle
        }
        s = this.formSections.find(x => x.dynamicComponentName === 'substance-form-simplified-names')
        if (s) {
            s.isHidden = this.simplifiedFormNameToggle
        }
    }
