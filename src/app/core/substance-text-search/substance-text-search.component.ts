  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @Input() styling?: "default" | "header" | "homePage" = "default";
  ...
  ngOnInit() {
    this.cvService
      .getDomainVocabulary("CODE_SYSTEM")
      .pipe(take(1))
      .subscribe((response) => {
        let resp;
    ...
    this.searchControl.valueChanges
      .pipe(
        ...
        switchMap((searchValue) => {
          ...
          const eventLabel =
            (!this.configService.environment.isAnalyticsPrivate &&
              searchValue) ||
            "search term";
      ...
      .subscribe(
        (response: SubstanceSuggestionsGroup) => {
          ...
          if (
            ...
            this.configService.configData.typeaheadFields
          ) {
         ...
         if (value === 'Display_Name') {
          ...
          this.suggestionsFields.sort(function (x, y) {
            return x === "Display_Name" ? -1 : y === "Display_Name" ? 1 : 0;
          });
          this.suggestionsFields.forEach((value, index) => {
            if (value === "Approval_ID") {
              if (
                ...
                this.suggestionsFields[index] = {
                  value: "Approval_ID",
                  display: this.configService.configData.approvalCodeName,
                };
              } else {
                this.suggestionsFields[index] = {
                  value: "Approval_ID",
          ...
          if (
            this.suggestionsFields != null &&
            this.suggestionsFields.length > 0
          ) {
            this.matOpen = true;
            this.opened.emit();
          }
  ...
  ngAfterViewInit() {
    this.searchContainerElement =
      this.element.nativeElement.querySelector(".search-container");
  }
  ...
  processSubstanceSearch() {
    ...
    const eventCategory = this.eventCategory || "substanceTextSearch";
    const eventLabel =
      (!this.configService.environment.isAnalyticsPrivate && searchTerm) ||
      "search term option";
  ...
  topSearchClean(searchTerm): string {
    if (searchTerm && searchTerm.length > 0) {
      ...
      const looksComplex =
        this.utilsService.looksLikeComplexSearchTerm(searchTerm);
