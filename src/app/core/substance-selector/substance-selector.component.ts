...
import { BatchSelectorDialogComponent } from "@gsrs-core/substance-batch/batch-selector-dialog/batch-selector-dialog.component";
import { BatchStorageService } from "@gsrs-core/substance-batch/batch-storage.service";
...
export class SubstanceSelectorComponent implements OnInit {
  ...
  */
  ...
  ngOnInit() {
    ...
    // Subscribe to batch count updates
  ...
  /**
   * Opens the batch selector dialog
   */
  openBatchSelector(): void {
    ...
    const dialogRef = this.dialog.open(BatchSelectorDialogComponent, {
      minWidth: "400px",
      maxWidth: "600px",
      data: {},
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result) {
        ...
        // For batch substances, interpret the structure to get a temporary ID for rendering
        if ((result as any).$$source === 'batch' && result.structure) {
          const mol = result.structure.molfile || result.structure.smiles;
          if (mol) {
            this.structureService.interpretStructure(mol).subscribe(
              (response) => {
                if (response && response.structure && response.structure.id) {
                  (this.selectedSubstance as any).$$tmpStructureId = response.structure.id;
                }
              },
              (error) => {
  ...
  openImageModal() {
    ...
    // For batch substances, use the temporary structure ID
